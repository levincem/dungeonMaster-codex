...
- Title-screen warm-up is now intentionally staggered: nearby dungeon data starts first, then gameplay visuals, `game_db` slices, wall overlays and render-core modules are warmed in idle/background waves so cold starts feel less frozen in `vite` dev and on slower machines.
