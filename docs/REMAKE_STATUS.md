...
- [src/GameRoot.tsx](/D:/DungeonMaster-codex/src/GameRoot.tsx) etale maintenant aussi le warm-up du titre en vagues idle: voisinage utile d'abord, puis visuels gameplay, slices `game_db`, overlays et modules de rendu coeur, avec un rattrapage differe si la machine reste idle plus longtemps
- le preload complet des niveaux en arriere-plan cede maintenant la main entre chaque niveau, ce qui reduit la sensation de freeze sur cold start et en `vite` dev tout en gardant le warm-up progressif du reste du donjon
